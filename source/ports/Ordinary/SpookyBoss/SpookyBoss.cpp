...
namespace {
static constexpr u32 kNsbmdMagic = 0x30444D42; // "BMD0"
static constexpr u32 kNsbcaMagic = 0x30414342; // "BCA0"
void* s_boss_model_file = nullptr;
void* s_boss_idle_file = nullptr;
u32 s_boss_model_ext = 0;
u32 s_boss_idle_ext = 0;
u32 s_boss_asset_generation = 0;
}  // namespace
...
bool SpookyBoss::loadResources() {
    // Resources are loaded explicitly in onCreate() through the DSi-aware loader.
    return true;
}
...
s32 SpookyBoss::onCreate(){
    ...
    if (s_boss_asset_generation != generation) {
        s_boss_model_file = nullptr;
        s_boss_idle_file = nullptr;
        s_boss_model_ext = 0;
        s_boss_idle_ext = 0;
        s_boss_asset_generation = generation;
    }
    ...
    if (s_boss_model_file == nullptr) {
        if (!ModRuntime_LoadValidatedFile("/z_new/Ordinary/actors/spookyBossLuigi.nsbmd", kNsbmdMagic,
                                          &s_boss_model_ext, &s_boss_model_file)) {
            Log() << "[ORDINARY][SpookyBoss] Failed to resolve/load boss model with BMD0 magic.\n";
            return false;
        }
    }
    if (s_boss_idle_file == nullptr) {
        if (!ModRuntime_LoadValidatedFile("/z_new/Ordinary/actors/spookyBossLuigiIdle.nsbca", kNsbcaMagic,
                                          &s_boss_idle_ext, &s_boss_idle_file)) {
            Log() << "[ORDINARY][SpookyBoss] Failed to resolve/load boss idle animation with BCA0 magic.\n";
            return false;
        }
    }
    bossModelID = s_boss_model_ext;
    idleAnimationID = s_boss_idle_ext;
    void* bossModelFile = s_boss_model_file;
    void* idleAnmFile = s_boss_idle_file;
    Log() << "[ORDINARY][SpookyBoss] Using model extFileId=" << bossModelID
          << " anim extFileId=" << idleAnimationID << "\n";

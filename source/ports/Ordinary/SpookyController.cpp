...
void SpookyController::onPrepareResources() {
	...
	if (staticTexReady && staticTexGeneration == generation) {
		return;
	}
	...
	if (!ModRuntime_LoadValidatedFile("/z_new/Ordinary/actors/Chaser_Static.nsbtx", kNsbtxMagic,
	                                  &extFileId, &nsbtxFile)) {
		staticTexReady = false;
		Log() << "[ORDINARY][SpookyController] Failed to resolve/load static texture with BTX0 magic.\n";
		return;
	}
...
void SpookyController::onRender() {
	...
	if (isRenderingStatic || staticDuration > 0) {
		...
		// Draw one tile of overdraw on each side to avoid edge seam artifacts.
		for (s32 row = -1; row < 4; row++) {
			for (s32 col = -1; col < 5; col++) {
				...
				const u32 texRow = scast<u32>((row % 3 + 3) % 3);
				const u32 texCol = scast<u32>((col % 4 + 4) % 4);
				u32 texID = nsbtxTexID[texRow][texCol];
		...
		for (u32 row = 0; row < 3; row++) {
			for (u32 col = 0; col < 4; col++) {
				nsbtxTexID[row][col] = (nsbtxTexID[row][col] + 1) % 4;
			}
		}
...
void SpookyController::stageSetup_hook() {
	ModRuntime_NotifySceneChange(kModRuntimeSceneEventAreaEnter);
	if (instance == nullptr) {
...
void SpookyController::stageDestroy_hook() {
	...
	if (Scene::nextSceneID != scast<u16>(SceneID::Stage)) {
		ModRuntime_NotifySceneChange(kModRuntimeSceneEventOverlayUnload);
		if (instance != nullptr) {
			instance->onDestroy();
			delete instance;
			instance = nullptr;
...
bool SpookyController::switchArea_hook() {
	ModRuntime_NotifySceneChange(kModRuntimeSceneEventSwitchArea);
